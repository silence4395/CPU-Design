...
con_expr returns [Exp v]
		...
		| a=con_expr ('+' {int op = BinOpExp.Plus;}
		             |'-' {int op = BinOpExp.Minus;}) b=con_expr 
		  {$v = BinOpExp($a.v, op, $b.v);}
		| a=con_expr ('<<' {int op = BinOpExp.Shl;}
		             |'>>' {int op = BinOpExp.Shr;}) b=con_expr 
		  {$v = BinOpExp($a.v, op, $b.v);}
		| a=con_expr ('<' {int op = BinOpExp.Less;} 
		             |'>' {int op = BinOpExp.Greater;}
		             |'<=' {int op = BinOpExp.LessOrEqual;}
		             |'>=' {int op = BinOpExp.GreaterOrEqual;}) b=con_expr
		  {$v = BinOpExp($a.v, op, $b.v);}
		| a=con_expr ('==' {int op = BinOpExp.Equal;} 
		             |'!=' {int op = BinOpExp.NotEqual}) b=con_expr
		  {$v = BinOpExp($a.v, op, $b.v);}
