...
type_specifier returns [Ty v]
		...
		| {Symbol sym = null; List<Ty> list1 = new ArrayList<Ty>(); List<Declors> list2 = new ArrayList<Declors>();}
		  b=struct_or_union (e=identifier {sym = symbol("name "+$e.v);})? 
		  '{' (c=type_specifier {list1.add($c.v);} d=declarators {list2.add($d.v);} ';')+ '}'
		  {$v = new RecordTy(sym, $b.v, list1, list2);}
		| b=struct_or_union e=identifier {$v = new RecordTy(symbol("name "+$e.v), $b.v);}
		;
...
declarator returns [Declor v]
		...
		| {List<Exp> list = new ArrayList<Exp>();}
		  a=plain_declarator ('[' c=logical_or_expression ']' {list.add($c.v);} )*
...
logical_or_expression returns [Exp v]
		: {List<Exp> list = new ArrayList<Exp>();} 
		  a=logical_and_expression {list.add($a.v);}
		  ('||' b=logical_and_expression {list.add($b.v);})*
		  {$v = new OpExp(list, OpExp.LogicalOr);}
		;

logical_and_expression returns [Exp v]
		: {List<Exp> list = new ArrayList<Exp>();} 
		  a=inclusive_or_expression {list.add($a.v);}
		  ('&&' b=inclusive_or_expression {list.add($b.v);})*
		  {$v = new OpExp(list, OpExp.LogicalAnd);}
		;

inclusive_or_expression returns [Exp v]
		: {List<Exp> list = new ArrayList<Exp>();}
		  a=exclusive_or_expression {list.add($a.v);}
		  ('|' b=exclusive_or_expression {list.add($b.v);})*
		  {$v = new OpExp(list, OpExp.Or);}
		;

exclusive_or_expression returns [Exp v]
		: {List<Exp> list = new ArrayList<Exp>();}
		  a=and_expression {list.add($a.v);}
		  ('^' b=and_expression {list.add($b.v);})*
		  {$v = new OpExp(list, OpExp.Xor);}
		;

and_expression returns [Exp v]
		: {List<Exp> list = new ArrayList<Exp>();}
		  a=equality_expression {list.add($a.v);}
		  ('&' b=equality_expression {list.add($b.v);})*
		  {$v= new OpExp(list, OpExp.And);}
		;

equality_expression returns [Exp v]
		: {List<Exp> list1 = new ArrayList<Exp>(); List<Integer> list2 = new ArrayList<Integer>();}
		  a=relational_expression {list1.add($a.v);} 
		  (b=equality_operator {list2.add(new Integer($b.v));}
		   c=relational_expression {list1.add($c.v);})*
		  {$v = new OpExp(list1, list2);}
		;
...
relational_expression returns [Exp v]
		: {List<Exp> list1 = new ArrayList<Exp>(); List<Integer> list2 = new ArrayList<Integer>();}
		  a=shift_expression {list1.add($a.v);} 
		  (b=relational_operator {list2.add(new Integer($b.v));}
		   c=shift_expression {list1.add($c.v);})*
		  {$v = new OpExp(list1, list2);}
		;
...
relational_operator returns [int v]
		...
		| '>' {$v = OpExp.Greater;}
		| '<=' {$v = OpExp.LessOrEqual;}
		| '>=' {$v = OpExp.GreaterOrEqual;}
		;

shift_expression returns [Exp v]
		: {List<Exp> list1 = new ArrayList<Exp>(); List<Integer> list2 = new ArrayList<Integer>();}
		  a=additive_expression{list1.add($a.v);}
		  (b=shift_operator {list2.add(new Integer($b.v));}
		   c=additive_expression {list1.add($c.v);})*
		  {$v = new OpExp(list1, list2);}
		;
...
additive_expression returns [Exp v]
		: {List<Exp> list1 = new ArrayList<Exp>(); List<Integer> list2 = new ArrayList<Integer>();}
		  a=multiplicative_expression {list1.add($a.v);}
		  (b=additive_operator {list2.add(new Integer($b.v));}
		   c=multiplicative_expression {list1.add($c.v);})*
		  {$v = new OpExp(list1, list2);}
		;
...
multiplicative_expression returns [Exp v]
		: {List<Exp> list1 = new ArrayList<Exp>(); List<Integer> list2 = new ArrayList<Integer>();}
		  a=cast_expression {list1.add($a.v);}
		  (b=multiplicative_operator {list2.add(new Integer($b.v));}
		   c=cast_expression {list1.add($c.v);})*
		  {$v = new OpExp(list1, list2);}
		;
...
character_constant returns [String v]
		: a=CHAR {$v = $a.text;}
		;
