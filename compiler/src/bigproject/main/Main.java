...
import bigproject.bintranslate.BinTranslate;
...
public class Main {
	...
	//	String filename = "input.c";
		...
		*/
		...
		writer.close();
		*/
		List<String> fincode = code.getCode();
		BinTranslate btranslate = new BinTranslate(fincode);
		btranslate.work();
		BufferedWriter writer = new BufferedWriter(new FileWriter("program.s"));
		btranslate.printToFile(writer);
		writer.flush();
		writer.close();
